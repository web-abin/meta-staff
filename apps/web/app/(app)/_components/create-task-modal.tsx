...
export function CreateTaskModal({ onClose }: { onClose: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      onClose();
      router.push(`/projects/${defaultWf?.id ?? ""}`);
      // 触发一次 ws 之外的 refresh：直接 router push 到项目页，
      // 项目页本身有 useEvents 订阅，AI 节点跑起来会自动刷出 task。
      void task;
    } catch (e) {
      setErr(String((e as Error).message ?? e));
      setBusy(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mt-5 p-3 rounded-md flex items-center justify-between" style={{ background: "var(--bg-soft)" }}>
          <div>
            <div className="text-[11px]" style={{ color: "var(--text-3)" }}>
              {t("new_task.workflow")}
            </div>
            <div className="text-[14px] font-medium mt-0.5">
              {defaultWf?.name ?? t("new_task.workflow_default")}
            </div>
          </div>
          {defaultWf?.is_default && (
            <span className="tag">{t("wf.list.default_tag")}</span>
          )}
        </div>
